...
func TestBreaking_DeprecationWithEarlySunset(t *testing.T) {
	...
	require.NoError(t, err)
	s2.Paths["/api/test"].Get.ExtensionProps.Extensions[diff.SunsetExtension] = toJson(t, civil.DateOf(time.Now().Add(1*time.Hour*24)).String())
...
func TestBreaking_DeprecationWithProperSunset(t *testing.T) {
	...
	s2.Paths["/api/test"].Get.ExtensionProps.Extensions[diff.SunsetExtension] = toJson(t, civil.DateOf(time.Now().Add(100*time.Hour*24)).String())
