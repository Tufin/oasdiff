...
func TestBreaking_DeprecationWithEarlySunset(t *testing.T) {
	...
	require.NoError(t, err)
	s2.Paths["/api/test"].Get.Extensions.Extensions[diff.SunsetExtension] = toJson(t, civil.DateOf(time.Now()).AddDays(9).String())
...
func TestBreaking_DeprecationWithProperSunset(t *testing.T) {
	...
	s2.Paths["/api/test"].Get.Extensions.Extensions[diff.SunsetExtension] = toJson(t, civil.DateOf(time.Now()).AddDays(10).String())
