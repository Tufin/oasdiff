...
func TestBreaking_ReqTypeStringToNumber(t *testing.T) {
	...
	require.Equal(t, "the request's body type/format changed from 'string'/'' to 'number'/''", errs[0].GetText())
}
...
func TestBreaking_ReqTypeNumberToString(t *testing.T) {
	...
	require.Len(t, errs, 1)
	require.Equal(t, checker.RequestBodyTypeChangedId, errs[0].GetId())
	require.Equal(t, "the request's body type/format changed from 'number'/'' to 'string'/''", errs[0].GetText())
}
...
func TestBreaking_ReqTypeNumberToInteger(t *testing.T) {
	...
	require.Len(t, errs, 1)
	require.Equal(t, checker.RequestBodyTypeChangedId, errs[0].GetId())
	require.Equal(t, "the request's body type/format changed from 'number'/'' to 'integer'/''", errs[0].GetText())
}
...
func TestBreaking_ReqTypeNumberToInt32(t *testing.T) {
	...
	require.Len(t, errs, 1)
	require.Equal(t, checker.RequestBodyTypeChangedId, errs[0].GetId())
	require.Equal(t, "the request's body type/format changed from 'number'/'' to 'integer'/'int32'", errs[0].GetText())
}
