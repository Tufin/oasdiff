			...
			for paramLocation, paramDiffs := range operationItem.ParametersDiff.Modified {
				for paramName, paramDiff := range paramDiffs {
					...
					result = append(result, BackwardCompatibilityError{
						...
						Level:     ERR,
						Text:      fmt.Sprintf("for the %s request parameter %s, the maxLength was decreased from %s to %s", ColorizedValue(paramLocation), ColorizedValue(paramName), maxLengthDiff.From, maxLengthDiff.To),
						Operation: operation,
						Path:      path,
						Source:    source,
